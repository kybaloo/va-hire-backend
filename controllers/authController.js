...
exports.socialRegister = async (req, res) => {
  try {
    const { email, firstname, lastname, socialProvider, socialId, profileData } = req.body;
  ...
  try {
    ...
    if (user) {
      ...
      if (provider !== 'auth0') {
        ...
        const normalizedProvider = provider === 'google-oauth2' ? 'google' : provider;
      ...
      user = new User({
        ...
        socialProviders: provider !== 'auth0' ? [{
          provider: normalizedProvider,
          id: socialProviderId,
          profile: {
            name: auth0User.name,
...
exports.completeProfile = async (req, res) => {
  try {
    ...
    const userId = req.auth.sub; // Using Auth0 subject identifier
