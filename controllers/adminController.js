...
exports.getUsers = async (req, res) => {
  try {
    // Check if user is admin using the database user from middleware
    const admin = req.dbUser;
    if (!admin || admin.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Only administrators can access this endpoint" });
    }
  ...
  try {
    ...
    res.status(200).json({
      ...
      courses: {
        total: totalCourses,
        enrollments:
          totalEnrollments.length > 0 ? totalEnrollments[0].count : 0,
      },
      finances: {
        ...
        totalRevenue:
          (projectRevenue.length > 0 ? projectRevenue[0].total : 0) +
...
exports.banUser = async (req, res) => {
  try {
    ...
    if (!admin || admin.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Only administrators can ban users" });
    }
...
exports.updateUserStatus = async (req, res) => {
  try {
    ...
    if (!admin || admin.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Only administrators can update user status" });
    }
...
exports.unbanUser = async (req, res) => {
  try {
    ...
    if (!admin || admin.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Only administrators can unban users" });
    }
